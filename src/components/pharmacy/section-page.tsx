...
import {
  ACTIONS, ActionDialog, EnquiryDialog, useSubmitAction,
  type ActionSpec, type SectionKey, SECTIONS,
} from "./actions";

export function PharmacySectionPage({ sectionKey }: { sectionKey: SectionKey }) {
  const section = SECTIONS.find((s) => s.key === sectionKey)!;
  const submit = useSubmitAction();
  const items = useWorkflow((s) => s.items.pharmacy);
  const { principal } = useAuth();
  const permissions = getDefaultModulePermissions("pharmacy");
  ...
  const openAction = (a: ActionSpec) => {
    if (a.readOnly) setEnquiryOpen(true);
    else setActive(a);
  };
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
            ...
            {section.actions.map((k) => {
              ...
              return (
                ...
                >
                  ...
                  {a.readOnly && (
                    <span className="relative mt-3 inline-flex items-center gap-1 rounded-full border border-border bg-muted/40 px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider text-muted-foreground">
                      Read-only
                    </span>
                  )}
                </button>
        ...
        <div>
          <div className="mb-3 flex items-end justify-between">
            ...
            <Link to="/pharmacy" className="text-xs text-muted-foreground hover:text-foreground">
              All pharmacy →
            </Link>
          </div>
          <Card className="p-3">
            ...
            <div className="mt-3 flex items-center justify-between border-t border-border pt-3 text-[11px] text-muted-foreground">
              ...
              <Link to="/pharmacy" className="inline-flex items-center gap-1 text-primary hover:underline">
...
export function sectionHead(section: (typeof SECTIONS)[number]) {
  return {
    meta: [
      { title: `${section.title} — Pharmacy · Impilo` },
