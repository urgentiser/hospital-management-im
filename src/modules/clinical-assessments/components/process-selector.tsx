...
import {
  clinicalAssessmentProcessGroups,
  clinicalAssessmentProcesses,
  type ClinicalAssessmentProcessDef,
  type ClinicalAssessmentProcessGroupKey,
} from "@/modules/clinical-assessments/workflows/process-registry";

type Props = { onLaunch: (process: ClinicalAssessmentProcessDef) => void };

export function ClinicalAssessmentProcessSelector({ onLaunch }: Props) {
  const [query, setQuery] = useState("");
  const [activeGroup, setActiveGroup] =
    useState<ClinicalAssessmentProcessGroupKey | "all">("all");
  const { principal } = useAuth();

  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase();
    return clinicalAssessmentProcesses.filter((p) => {
  ...
  const grouped = useMemo(() => {
    const map = new Map<ClinicalAssessmentProcessGroupKey, ClinicalAssessmentProcessDef[]>();
  ...
  return (
    <section className="space-y-4">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold tracking-tight">Clinical assessment processes</h2>
          <p className="text-sm text-muted-foreground">
            Launch a guided clinical assessment process. Grouped by operational area.
          </p>
        </div>
        <div className="relative w-full sm:w-72">
          <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
          <Input
            aria-label="Search clinical assessment processes"
      ...
      <div className="flex flex-wrap gap-1.5">
        <GroupChip active={activeGroup === "all"} onClick={() => setActiveGroup("all")} label="All" count={clinicalAssessmentProcesses.length} />
        {clinicalAssessmentProcessGroups.map((g) => (
          <GroupChip
            key={g.key}
            active={activeGroup === g.key}
            onClick={() => setActiveGroup(g.key)}
            label={g.title}
            count={clinicalAssessmentProcesses.filter((p) => p.group === g.key).length}
          />
        ))}
      </div>
...
}: { process: ClinicalAssessmentProcessDef; allowed: boolean; onLaunch: (p: ClinicalAssessmentProcessDef) => void }) {
