...
const config: ModuleConsoleConfig = {
  ...
  sections: [
    ...
    {
      key: "endpoints", title: "Endpoints & Schemas", tagline: "Register · validate",
      description: "Register connected systems and validate event schemas.",
      ...
      actions: [
        ...
        { key: "validate", label: "Validate schema", icon: Bug, hint: "Run schema validation on a topic", kind: "Schema", startStatus: "verified",
          fields: [
  ...
  businessFlow: {
    moduleKey: "integrations",
    title: "Emit · Retry · Resolve",
    purpose: "Every domain event travels the integration channel reliably. Emit, monitor, retry on transient failure, dead-letter on poison, then resolve or replay.",
    ...
    events: [
      ...
      "EventResolved", "EndpointRegistered", "SchemaValidated",
    ],
    handoffs: ["Failed Messages", "Audit Trail", "System Health"],
    globalRules: [
      "Every emit is idempotent — a message ID identifies duplicates.",
      "Retry with exponential backoff; abandon to DLQ after configured max attempts.",
      "Every message is schema-validated against the topic schema.",
      "Correlation IDs must be preserved end-to-end.",
      "Every action is automatically audited.",
    ],
    acceptance: [
      "Emit an event, see it delivered and traced by correlation ID.",
      "Inject a transient failure and observe retry then delivery.",
      "Route a poison message to DLQ and resolve or replay it.",
    ],
    steps: [
      { key: "publish", title: "Emit event", description: "A domain module emits an event with correlation ID and message body.",
        fields: [
          { name: "topic", label: "Topic", required: true },
          { name: "correlation", label: "Correlation ID", required: true },
          { name: "message", label: "Message preview", type: "textarea" },
        ], events: ["EventPublished"] },
      { key: "validate", title: "Schema validation", description: "The integration channel validates the message against the registered schema.",
        checklist: ["Schema version matches", "Required fields present", "No unknown fields at strict endpoints"] },
      ...
      { key: "triage", title: "Triage from DLQ", description: "Operators inspect payloads and decide to resolve, replay or discard.",
        checklist: ["Message inspected", "Root cause identified", "Ticket raised if code fix required"] },
