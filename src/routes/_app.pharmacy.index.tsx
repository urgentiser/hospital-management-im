...
import { ArrowRight, Activity, Pill, PackageCheck, Ban } from "lucide-react";
import { Card, StatusChip } from "@/components/app-shell";
import { useWorkflow } from "@/lib/workflow-store";
import { SECTIONS, ACTIONS } from "@/components/pharmacy/actions";

export const Route = createFileRoute("/_app/pharmacy/")({
  head: () => ({
    meta: [
      { title: "Pharmacy — Impilo" },
      { name: "description", content: "Dispensing, compounding, labels, ward, theatre and retail workflows in one calm control plane." },
    ],
  }),
    ...
    <>
      <div className="mb-8 grid gap-4 lg:grid-cols-[1.4fr_1fr]">
        <Card className="relative overflow-hidden p-6">
          ...
          <div className="relative">
            ...
            <h2 className="mt-3 max-w-xl font-display text-2xl leading-tight tracking-tight sm:text-3xl">
              A calm command centre for every pharmacy workflow.
            </h2>
            <p className="mt-2 max-w-xl text-sm text-muted-foreground">
              {totalActions} curated actions across {SECTIONS.length} pillars — from bedside dispensing to retail sales and compounding — with a live audit trail.
            </p>
            <div className="mt-5 flex flex-wrap gap-2">
              <Link
                to={"/pharmacy/dispensing" as "/pharmacy"}
              ...
              >
                Start dispensing <ArrowRight className="h-4 w-4" />
              </Link>
              <Link
                to={"/pharmacy/enquiry" as "/pharmacy"}
        ...
        <div className="grid grid-cols-2 gap-4">
          <StatCard icon={Pill} label="Ordered" value={stats.ordered} accent="from-primary/30 to-transparent" />
          <StatCard icon={PackageCheck} label="Picked" value={stats.picked} accent="from-indigo-500/30 to-transparent" />
          <StatCard icon={Activity} label="Dispensed" value={stats.dispensed} accent="from-emerald-500/30 to-transparent" />
          <StatCard icon={Ban} label="Cancelled" value={stats.cancelled} accent="from-rose-500/30 to-transparent" tone="destructive" />
        </div>
      </div>
      ...
      <div className="mb-3 flex items-end justify-between">
        <div>
          ...
          <h2 className="mt-1 font-display text-xl tracking-tight">Pharmacy sections</h2>
        </div>
      </div>
              ...
              <div className="relative mt-4 flex flex-wrap gap-1.5">
                {previewActions.map((k) => (
                  <span key={k} className="rounded-md border border-border bg-background/60 px-1.5 py-0.5 text-[10px] text-muted-foreground">
                    {ACTIONS[k].label.replace(/^Dispense |^Maintain |^Manage |^Fulfill /, "")}
                  </span>
                ))}
      ...
      <div className="mt-10">
        <div className="mb-3 flex items-end justify-between">
          <div>
            ...
            <h2 className="mt-1 font-display text-xl tracking-tight">Latest pharmacy activity</h2>
          </div>
        </div>
