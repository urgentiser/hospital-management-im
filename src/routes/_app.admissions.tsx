  ...
  sections: [
    {
      ...
      tagline: "Admit · move · discharge",
      description: "Every admission action Impilo users run day-to-day — admit, transfer, discharge, cancel, undischarge and register a birth.",
      icon: BedDouble,
      accent: "from-emerald-500/25 via-teal-500/15 to-transparent",
      ring: "ring-emerald-400/30",
      actions: [
        { key: "admit-patient", label: "Admit Patient", icon: UserPlus, hint: "Standard admission from a preadmission or ready patient", kind: "Admit Patient", startStatus: "admitted",
          fields: [
        ...
        { key: "direct-admission", label: "Direct Admission", icon: HeartPulse, hint: "Admit a walk-in without a preadmission record", kind: "Direct Admission", startStatus: "admitted",
          fields: [
            { name: "patient", label: "Patient", required: true },
            { name: "facility", label: "Facility", required: true },
        ...
        { key: "no-auth-admission", label: "No-Authorisation Admission", icon: ShieldOff, hint: "Admit without authorisation — follow-up work is created", kind: "No-Auth Admission", startStatus: "pending",
          fields: [
            { name: "patient", label: "Patient", required: true },
            { name: "facility", label: "Facility", required: true },
            { name: "ward", label: "Ward", required: true },
            { name: "practitioner", label: "Admitting practitioner", required: true },
            { name: "reason", label: "Reason no authorisation on file", type: "textarea", required: true },
          ], destructive: true },
        { key: "move-to-ward", label: "Move to Ward", icon: ArrowRightLeft, hint: "Internal ward or bed transfer", kind: "Move to Ward", startStatus: "transferred",
          fields: [
            { name: "patient", label: "Patient", required: true },
            { name: "fromWard", label: "From ward" },
            { name: "toWard", label: "To ward", required: true },
            { name: "toBed", label: "To bed" },
            { name: "reason", label: "Reason", type: "textarea" },
          ]},
        { key: "patient-location", label: "Patient Location", icon: MapPin, hint: "Look up where an inpatient currently is", kind: "Patient Location", startStatus: "active",
          fields: [{ name: "patient", label: "Patient", required: true }] },
        { key: "discharge-patient", label: "Discharge Patient", icon: LogOut, hint: "Complete a discharge with disposition", kind: "Discharge Patient", startStatus: "discharged",
          fields: [
            { name: "patient", label: "Patient", required: true },
            { name: "dischargedAt", label: "Discharge date/time", placeholder: "YYYY-MM-DD HH:mm" },
            { name: "disposition", label: "Disposition", placeholder: "Home / Step-down / Transfer" },
            { name: "notes", label: "Clinical notes", type: "textarea" },
          ]},
        { key: "cancel-admission", label: "Cancel Admission", icon: Ban, hint: "Cancel before the patient occupied the bed", kind: "Cancel Admission", startStatus: "cancelled",
          fields: [
            { name: "patient", label: "Patient", required: true },
            { name: "reason", label: "Reason", required: true, type: "textarea" },
          ], destructive: true },
        { key: "discontinue-admission", label: "Discontinue Admission", icon: StopCircle, hint: "Stop an in-progress admission", kind: "Discontinue Admission", startStatus: "discontinued",
          fields: [
            { name: "patient", label: "Patient", required: true },
            { name: "reason", label: "Reason", required: true, type: "textarea" },
          ], destructive: true },
        { key: "undischarge-patient", label: "Undischarge Patient", icon: Undo2, hint: "Reverse a discharge (elevated permission)", kind: "Undischarge Patient", startStatus: "admitted",
          fields: [
            { name: "patient", label: "Patient", required: true },
            { name: "reason", label: "Reason", required: true, type: "textarea" },
          ], destructive: true },
        { key: "register-birth", label: "Register Birth", icon: Baby, hint: "Register a neonate under the mother's admission", kind: "Register Birth", startStatus: "admitted",
          fields: [
